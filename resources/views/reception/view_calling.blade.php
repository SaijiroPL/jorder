<div class="rd_pay_modal">
    <div style="padding: 30px 0 0 30px;">
        <h5><span class="ml-3 fs-30" style="font-weight: 700;">{{ __('reception.modal_title_calling') }}</span></h5>
    </div>
                        ...
                        <div class="row w-100 p-0 m-0" style="height: 125px;">
                            <div class="col-lg-2 pr-0 col-xl-2">
                                <div class="row" style="margin-left: -12px;">
                                    <p class="red-text font-weight-bold ml-3 fs-23" id="time_call_{{$kk}}">0分</p>
                                </div>
                            ...
                            <div class="col-lg-2 pr-0 col-xl-2">
                                @if($order->attend_time == null)
                                    <div class="attending fs-25" onclick="attend({{ $order->calling_table_id }})"><b>{{ __('reception.ATTENDING') }}</b><br>
                                        <img src="{{ asset('img/Group728white.png') }}" style="height:18px; margin-left: 9px;">
                                    </div>
                                @else
                                    <div class="attended fs-25"><b>{{ __('reception.ATTENDED') }}</b></div>
                                @endif
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function myTimer() {
        ...
        for(var i=0;i<order_side.length;i++){
            ...
            if(duration == 0) {
                document.getElementById("time_call_" + i).innerHTML = 'テイクアウト';
            } else if(duration == 1) {
                ...
                if(elapsed_time > 0) {
                    document.getElementById("time_call_" + i).innerHTML = elapsed_time + '分';
                }
                else {
                    document.getElementById("time_call_" + i).innerHTML = '0分';
                }
            } else if(duration == '2') {
                ...
                document.getElementById("time_call_" + i).innerHTML = '無制限';
            }
